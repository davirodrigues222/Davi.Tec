...
export const HistoricoServicos: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      {/* TEMPLATE DE IMPRESSÃO PROFISSIONAL (SEM CHECKLIST) */}
      {osParaImprimir && (
        <div className="hidden print:block text-slate-900 font-sans p-6 bg-white leading-relaxed text-left max-w-[210mm] mx-auto text-[11px]">
          ...
          <div className="border-b-2 border-slate-900 pb-4 mb-4 flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-black tracking-tight text-slate-900 uppercase">{dadosLoja.nomeLoja}</h1>
              <p className="text-xs text-slate-600 font-semibold">{dadosLoja.subtitulo}</p>
              <p className="text-[10px] text-slate-500 mt-1">
                CNPJ: {dadosLoja.cnpjCpf || "Não configurado"} | Tel: {dadosLoja.telefone || "Não configurado"} | End: {dadosLoja.endereco || "Não configurado"}
              </p>
            </div>
            <div className="text-right border-l-2 border-slate-300 pl-4">
              <div className="bg-slate-100 px-4 py-2 rounded-lg border border-slate-300">
                ...
                <span className="text-lg font-black font-mono text-blue-900">#{osParaImprimir.numero_os}</span>
              </div>
              <div className="text-[10px] text-slate-600 mt-2 space-y-0.5">
                <p><strong>Abertura:</strong> {osParaImprimir.data_abertura ? new Date(osParaImprimir.data_abertura).toLocaleDateString("pt-BR") : '-'}</p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="border border-slate-300 rounded-lg p-3 bg-slate-50/50">
              ...
              <p><strong>Nome:</strong> {osParaImprimir.cliente?.nome}</p>
              <p><strong>WhatsApp:</strong> {osParaImprimir.cliente?.whatsapp || "Não informado"}</p>
              <p><strong>CPF/CNPJ:</strong> {osParaImprimir.cliente?.cpfCnpj || "Não informado"}</p>
            </div>
            <div className="border border-slate-300 rounded-lg p-3 bg-slate-50/50">
              <h3 className="font-bold uppercase text-slate-700 border-b border-slate-200 pb-1 mb-2 text-xs">📱 Dados do Aparelho</h3>
              <p><strong>Modelo:</strong> {osParaImprimir.aparelho?.modelo}</p>
              <p><strong>IMEI / S/N:</strong> {osParaImprimir.aparelho?.imei1 || "Não informado"}</p>
              <p><strong>Senha:</strong> {osParaImprimir.aparelho?.senhaDesbloqueio || "Não informada"}</p>
            </div>
          </div>
          ...
          <div className="border border-slate-300 rounded-lg p-3 mb-4 flex justify-between items-center bg-slate-100 font-bold">
            <div>
              <span>Forma de Pagamento: {(osParaImprimir as any).formaPagamento || 'PIX'} ({(osParaImprimir as any).parcelas || 1}x)</span>
            </div>
            <div className="text-sm text-blue-900">
              Valor Total: R$ {(osParaImprimir.orcamentoCalculado?.valorTotalOrcamento || 0).toFixed(2)}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 pt-6 mt-6 border-t border-slate-300 text-center">
            ...
            <div>
              <div className="border-b border-slate-400 pb-8 mb-1"></div>
              <p className="font-bold text-slate-700">{osParaImprimir.cliente?.nome}</p>
